...
def make_pdf_cover(content):
    ...
    if not os.path.exists(g_env):
        print("未发现测试环境结果文件")
        content.append(PdfMaker.draw_little_title('未发现测试环境结果文件', 12, colors.red))
        return
    ...
    content.append(PdfMaker.draw_table(home_table_style, totalWidth=500, rates=[2,7], *data))
...
def make_pdf_test_env(content):
    ...
    content.append(PdfMaker.draw_table(extra_style, totalWidth=total_width, rowHeight=27, rates=[1,1,3], *data))
...
def make_pdf_lib(content):
    ...
    if not os.path.exists(g_lib):
        print("未发现运行库检查结果文件")
        content.append(PdfMaker.draw_little_title('未发现运行库检查结果文件', 12, colors.red))
        return
    ...
    lib_table_style = default_style + [
            ('FONTSIZE', (0, 0), (-1, 1), 15),  # 第一,二行的字体大小
            ('FONTSIZE', (0, 2), (-1, -1), 10),  # 第三行到最后一行的字体大小
    ...
    is_merge = False
    while i < row:
        if new_data[i][1] == "版本要求":
            lib_table_style.append(('BACKGROUND', (1, i), (-1, i), table_background_color))
            lib_table_style.append(('SPAN', (2, i), (-1, i)))
            lib_table_style.append(('SPAN', (2, i+1), (-1, i+1)))
        if new_data[i][1] == "二进制包名":
            lib_table_style.append(('BACKGROUND', (1, i), (-1, i), table_background_color))
            is_merge = True

        
        if new_data[i][1] == "共享对象名":
            lib_table_style.append(('BACKGROUND', (1, i), (-1, i), table_background_color))
        ...
        if new_data[i][-1] == "不通过":
            lib_table_style.append(('TEXTCOLOR', (0, i), (-1, i), colors.red))
   
        i += 1

    content.append(PdfMaker.draw_mul_table(lib_table_style, totalWidth=total_width, rates=[2,3,7,2,1], *new_data))
    while row <len(data) :
        i = 0     
        new_data = data[row:row+15]
        other_style = [('FONTSIZE', (0, 0), (-1, -1), 10),]
        other_style = other_style + default_style
        while i < len(new_data):
            if new_data[i][1] == "版本要求":
                other_style.append(('BACKGROUND', (1, i), (-1, i), table_background_color))
                other_style.append(('SPAN', (2, i), (-1, i)))
                other_style.append(('SPAN', (2, i+1), (-1, i+1)))
            ...
            if is_merge:
                other_style.append(('SPAN', (2, i), (3, i)))

            if new_data[i][-1] == "不通过":
                other_style.append(('TEXTCOLOR', (1, i), (-1, i), colors.red))
            i += 1
        content.append(PdfMaker.draw_mul_table(other_style, totalWidth=total_width, rates=[2,3,7,2,1], *new_data))
        row += 15
    ...
    result_style = default_style + [
            ('FONTSIZE', (0, 0), (-1, 0), 12),  # 第一,二行的字体大小
            ('FONTSIZE', (0, 2), (-1, -1), 10),  # 第三行到最后一行的字体大小
            ('BACKGROUND', (0, 0), (-1, 0), table_background_color),  # 设置第一行背景颜色
        ]
    for level in result_map:
        pack_pass_num = result_map[level]["pack_require_num"] - result_map[level]["pack_no_pass_num"]
        if result_map[level]["pack_require_num"] != 0:
            
            pack_rate = '{:.2f}%'.format(pack_pass_num / result_map[level]["pack_require_num"] * 100)
        else:
            pack_rate = 0

        #print(result_map[level]["shard_require_num"])
        if result_map[level]["shard_require_num"] != 0:
            shard_rate = '{:.2f}%'.format(result_map[level]["shard_pass_num"] / result_map[level]["shard_require_num"] * 100)
        else:
            shard_rate = 0

        
        table_content = (level, "包", result_map[level]["pack_require_num"], pack_pass_num, pack_rate)       
        data.append(table_content)

    
        table_content = (level, "共享对象", result_map[level]["shard_require_num"], result_map[level]["shard_pass_num"], shard_rate)  
        data.append(table_content)
        ...
        i += 2
    content.append(PdfMaker.draw_table(result_style, totalWidth=total_width, rates=[1,1,1,1,1], *data))
...
def make_pdf_fs(content):
    ...
    if not os.path.exists(g_fs):
        ...
        content.append(PdfMaker.draw_little_title('未发现文件系统层次结构检查结果文件', 12, colors.red))
        return 
    ...
    with open(g_fs,'r',encoding='utf8') as fp:
        ...
        for r in json_result:
            ...
            if r["result"] == "pass":
                ...
                result = "不符合"
                fs_table_style.append(('TEXTCOLOR', (0, i), (-1, i), colors.red))

            table_content = (r["FS_name"], exist, r["file_permissions"], result)         
            data.append(table_content)

            i += 1
            require_num += 1

    content.append(PdfMaker.draw_table(fs_table_style, totalWidth=total_width, rates=[1,2,2,1], *data))
    content.append(Spacer(1, 10))
    ...
    result_table_style = default_style + [
                ('FONTSIZE', (0, 0), (-1, 0), 12),  # 第一行的字体大小
                ('FONTSIZE', (0, 1), (-1, -1), 10),  # 第二行到最后一行的字体大小
                ('BACKGROUND', (0, 0), (-1, 0), table_background_color),  # 设置第一行背景颜色
            ]
    content.append(PdfMaker.draw_table(result_table_style, totalWidth=total_width, rates=[1,1,1], *data))
...
def make_pdf_cmd(content):
    ...
    if not os.path.exists(g_cmd):
        print("未发现常用命令检查结果文件")
        content.append(PdfMaker.draw_little_title('未发现常用命令检查结果文件', 12, colors.red))
        return
    ...
    with open(g_cmd,'r',encoding='utf8') as fp:
        ...
        for r in json_result:
            ...
            if r["result"] == "pass":
                ...
                result = "不符合"
                cmd_table_style.append(('TEXTCOLOR', (0, i), (-1, i), colors.red))
    ...
    content.append(PdfMaker.draw_table(cmd_table_style, totalWidth=total_width, rates=[1,4,2,2,2], *data))
...
def make_pdf_service(content):
    ...
    if not os.path.exists(g_server):
        print("未发现服务检查结果文件")
        content.append(PdfMaker.draw_little_title('未发现服务检查结果文件', 12, colors.red))
        return
    ...
    with open(g_server,'r',encoding='utf8') as fp:
        ...
        for r in json_data :
            ...
            if result == "pass":
                ...
                result = "不通过"
                service_style.append(('TEXTCOLOR', (0, i), (-1, i), colors.red))
            table_content = (r, result)         
            data.append(table_content)
            i += 1

    content.append(PdfMaker.draw_table(service_style, totalWidth=total_width, rates=[1,1], *data))
...
def make_pdf_conclusion(content):
    ...
    with open(g_env,'r',encoding='utf8') as fp:
        json_data = json.load(fp)
        print(g_isConform)
        text = '经测试，{}送测的{}-{}({}){}《操作系统应用编程接口要求》。'.format(json_data["送测单位"],json_data["测试对象"]["系统名称"],json_data["测试对象"]["版本"], 
                                                                                    json_data["环境配置"]["CPU指令集"][0], g_isConform)
