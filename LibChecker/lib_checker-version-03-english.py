...
def get_platform_info():
    global g_ostype
    print("Start APIChecker ---> LibChecker !!!")
...
#    print(f_dict)
    print("\tStandard Summary Info:")
    print("\t\tserial number: %s" % f_dict['std_description']['std_number'])
    print("\t\tfile name: %s" % f_dict['std_description']['std_name'])

    print("\tLibChecker Summary Info:")
    print("\t\tcheck location: %s" % f_dict['libs']['lib_location'])
...
#    p_srcpkgnam.close()
    ...
    print("\t\timplement package info: ")

    if (len(srcpkgver) == 0):
        print("\t\t\t\t", "not found this package")
        print("\t\tshared libraries info:")
    else:
        print("\t\t\t\tsrc_pkgname -> ", src_pkgname.ljust(20),"version -> ",srcpkgver)
        print("\t\tshared libraries info:")
...
#    print('chapter:'.ljust(10),'name:'.ljust(20), 'level:'.ljust(5), 'version:'.ljust(5))
    ...
    for chapter_class in g_jsonfile_dict['libs']['category']:
       ...
       # for key in l_tmp_dict:
            print("\tstart checking ", '<',  key,'>', )
            print("\t\tsections_number -->", l_tmp_dict[key]['sections_number'], key)
#            print("\t\tlib_description -->", l_tmp_dict[key]['lib_description'])
            
            
            print("\t\tnecessity       -->", l_tmp_dict[key]['necessity'][g_ostype])
            print("\t\tstandand reference info:")
            print("\t\t\t\tstd_pkgname -> " ,key.ljust(20),"version -> ", l_tmp_dict[key]['version'][g_ostype].ljust(5))
            if (len(l_tmp_dict[key]['version'][g_ostype]) == 0):
                print("\t\timplement package info: ")
                print("\t\t\t\t\"not find this package\" ")
            else:
            ...
            for list1_item in g_jsonfile_dict['libs']['category'][chapter_class]['packages'][key]['share_objs'][g_ostype]:
                print("\t\t\t\tlib_soname -> ",list1_item, )
                lib_result = check_sharelib_info('/lib', list1_item)
                print("\t\t\t\tlib_locate -> ",lib_result,)
...
#    check_per_so_info()
    ...
    print("=============================================================================================================")
    print("End check at [",time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()))
    print("\tPackage:")
    print("\t\tTotal:", g_counter_flags['pkg_counter']['total'], "|  Passed:", g_counter_flags['pkg_counter']['passed'], "| Failed:", g_counter_flags['pkg_counter']['total'] - g_counter_flags['pkg_counter']['passed'])
    print("\tLibrary:")
#    print("\t\t\ttotal:", g_counter_flags['lib_counter']['total'], "|  passed:", g_counter_flags['lib_counter']['passed'])
    print("\t\tTotal:", g_counter_flags['lib_counter']['total'], "|  Passed:", g_counter_flags['lib_counter']['passed'], "| Failed:", g_counter_flags['lib_counter']['total'] - g_counter_flags['lib_counter']['passed'])
    print("=============================================================================================================")
